...
class DeepfmModel(nn.Module):
    def __init__(self):
        ...
        self.out_layer   = nn.Linear(self.emb_size1+self.emb_size2+self.nn_size[-1], 1)
    ...
    def forward(self, single_fea, multi_fea, mask_fea):
        ...
        # nn part
        nn_out    = self.nn_part(fea2st.view([-1, (self.single_size+self.multi_size)*self.emb_size2]))        # [b, 32]
        # fm part
        fm1st_out = self.fm_1st_part(fea1st)    # [b, emb_size1] = [b, 1]
        fm2st_out = self.fm_2st_part(fea2st)    # [b, emb_szie2] = [b, 64]
    
        out_fea   = torch.cat([fm1st_out, fm2st_out, nn_out], dim=1)     # [b, 1+64+32]    
