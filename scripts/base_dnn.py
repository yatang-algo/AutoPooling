...
class nnModel(nn.Module):
    def __init__(self):
        ...
        self.batchNorm  = nn.ModuleDict({name: nn.BatchNorm1d(self.emb_size) for name in self.multi_name})
    ...
    def pooling_fun(self, masked_emb, pooling_name='sum', atten_w=None):
        """ 
        pooling_name : {sum, mean, max, min, atten, korder, pca}
    ...
    def forward(self, single_fea, multi_fea, mask_fea):
        ...
        for name, feature, mask in zip(self.multi_name, multi_fea, mask_fea):
            ...
            if self.pooling_name == 'atten':
                ...
                pooled_emb  = self.pooling_fun(masked_emb, pooling_name=self.pooling_name)      # [b, emb_size]
            if self.useBN == True:
                pooled_emb = self.batchNorm[name](pooled_emb)
            emb2.append(pooled_emb)
        emb2 = torch.cat(emb2, dim=1)
        
        # nn part
        totoal_fea = torch.cat([emb1, emb2], dim=1)      
        output = torch.relu(self.layer1(totoal_fea))
