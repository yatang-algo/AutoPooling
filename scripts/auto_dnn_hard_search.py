...
import read_batch
import auc
import auto_dnn_hard_retrain as RetrainModel

class AutoPoolSearch(nn.Module):
    def __init__(self):
        super(AutoPoolSearch, self).__init__()
        self.threads  = None
        self.needinit = True
        self.cuda   = 'cuda'
        ...
        self.batch_size     = 512
        self.learning_rate  = [0.005, 1e-3]
        self.epoch          = 2
        self.t_batch        = 5
        self.temperature    = 0.005
        self.global_step    = 0
        self.begin_step     = 2000

        self.pooling_module = ["sum", "mean", "max", "min", "atten", "korder"]
        ...
        self.sumEmb     = nn.ModuleDict({name: nn.Embedding(self.fea_size[name], self.emb_size) for name in self.fea_size})
        self.meanEmb    = nn.ModuleDict({name: nn.Embedding(self.fea_size[name], self.emb_size) for name in self.fea_size})
        self.maxEmb     = nn.ModuleDict({name: nn.Embedding(self.fea_size[name], self.emb_size) for name in self.fea_size})
        self.minEmb     = nn.ModuleDict({name: nn.Embedding(self.fea_size[name], self.emb_size) for name in self.fea_size})
        self.attenEmb   = nn.ModuleDict({name: nn.Embedding(self.fea_size[name], self.emb_size) for name in self.fea_size})
        self.korderEmb      = nn.ModuleDict({name: nn.Embedding(self.fea_size[name], self.emb_size) for name in self.fea_size})
        self.EmbDict    = {
            'sum': self.sumEmb, 'mean': self.meanEmb, 
            'max': self.maxEmb, 'min': self.minEmb, 
            'atten': self.attenEmb, 'korder': self.korderEmb
            }
    ...
    def gumbel_trick(self, alpha):
        gg    = -torch.log(-torch.log(torch.FloatTensor(len(self.pooling_module)).uniform_(1e-6, 1.0))).to(self.device)
        alpha = torch.relu(alpha).to(self.device)
        temperature = torch.max(torch.tensor([self.temperature, 1 - 0.000024*self.global_step], dtype=torch.float32).to(self.device))
        pp    = torch.softmax((torch.log(alpha) + gg) / temperature, dim=0)
        return pp
    
    def pooling_fun(self, masked_emb, atten_w, alpha, name):
        """ 
        pooling_module : {sum, mean, max, min, atten, korder}
        masked_emb     : a list of masked embedding with the size of [b, max_fea_num, emb_size]
        atten_w        : [b, max_fea_num, 1]
        narmal_fum     : layer normalization on embedding dim
        alpha          : structure parameters in [6,]
        """
        emb_size  = masked_emb[0].shape[-1]
        sum_pool  = torch.sum(masked_emb[0], dim=1, keepdim=True)         # [b, 1, emb_size]
        sum_pool  = self.sumBN[name](sum_pool.view([-1, emb_size])).view([-1, 1, emb_size])
        mean_pool = torch.mean(masked_emb[1], dim=1, keepdim=True)         # [b, 1, emb_size]
        mean_pool = self.meanBN[name](mean_pool.view([-1, emb_size])).view([-1, 1, emb_size])

        maxL2_emb = torch.sum(torch.square(masked_emb[2]), dim=-1)
        minL2_emb = torch.sum(torch.square(masked_emb[3]), dim=-1)
        ...
        max_mask  = torch.zeros(masked_emb[2].shape[1], masked_emb[2].shape[0]).to(self.device).scatter(0, max_index.view(1,-1), 1)
        max_mask  = max_mask.transpose(1, 0).view(masked_emb[2].shape[0], masked_emb[2].shape[1], 1)
        min_mask  = torch.zeros(masked_emb[3].shape[1], masked_emb[3].shape[0]).to(self.device).scatter(0, min_index.view(1,-1), 1)
        min_mask  = min_mask.transpose(1, 0).view(masked_emb[3].shape[0], masked_emb[3].shape[1], 1)
        
        max_pool   = torch.sum(masked_emb[2] * max_mask, dim=1, keepdim=True)           # [b, 1, emb_size]
        max_pool   = self.maxBN[name](max_pool.view([-1, emb_size])).view([-1, 1, emb_size])
        min_pool   = torch.sum(masked_emb[3] * min_mask, dim=1, keepdim=True)           # [b, 1, emb_size]
        min_pool   = self.minBN[name](min_pool.view([-1, emb_size])).view([-1, 1, emb_size])
        atten_w    = torch.softmax(atten_w, dim=1)
        atten_pool = torch.sum(torch.mul(masked_emb[4], atten_w), dim=1, keepdim=True)  # [b, 1, emb_size]
        atten_pool = self.attenBN[name](atten_pool.view([-1, emb_size])).view([-1, 1, emb_size])
        
        squared_sum  = torch.square(torch.sum(masked_emb[5], dim=1, keepdim=True))    # [b, 1, emb_size]
        sumed_square = torch.sum(torch.square(masked_emb[5]), dim=1, keepdim=True)    # [b, 1, emb_size]
        ...
        normaled_pool = torch.cat([
            sum_pool, mean_pool, max_pool, min_pool, atten_pool, korder_pool
            ], dim=1).view([-1, len(self.pooling_module), emb_size])
        
        weight = self.gumbel_trick(alpha).view([1, len(self.pooling_module), 1])
    ...
    def forward(self, single_fea, multi_fea, mask_fea):
        ...
        for name, feature, mask in zip(self.multi_name, multi_fea, mask_fea):
            ...
            pool_emb_list = []
            for pool_name in self.pooling_module:
                pool_emb_list.append(self.mask_fun(self.EmbDict[pool_name][self.multi_name[name]](feature), mask))   # [b, max_fea_num, emb_size]
            pooled_emb = self.pooling_fun(
                pool_emb_list, atten_w=atten_w, alpha=self.alpha[name], name=name
                )     # [b, emb_size]
            emb2.append(pooled_emb)
        emb2 = torch.cat(emb2, dim=1)

        # nn part
        totoal_fea = torch.cat([emb1, emb2], dim=1)
    ...
    def fit(self):
        ...
        for epoch in range(self.epoch):
            ...
            for i, df in enumerate(data):
                ...
                loss.backward()
                if self.global_step >= self.begin_step and (i+1) % self.t_batch == 0:
                    optimizer2.step()
                else:
                    optimizer1.step()
        ...
        # get hard select of all slot
        name_list = []
        for name in self.multi_name:
            alpha  = model.alpha[name]
            pp_loc = torch.max(self.gumbel_trick(alpha), dim=0).indices.item()
            name_list.append(self.pooling_module[pp_loc])
        print(name_list)
        return name_list
...
if __name__ == "__main__":
    ...
    print('===== train stage 1 =====')
    themodel  = AutoPoolSearch()
    start     = time.time()
    name_list = themodel.fit()
    end       = time.time()
    print('===== train stage1 finished in {:d}min {:d}s ====='.format(int((end-start) // 60), int((end-start) % 60)))

    print('===== train stage 2 =====')
    themodel = RetrainModel.AutoModelRetrain()
    start    = time.time()
